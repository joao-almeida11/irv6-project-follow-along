...
const App = AppImport.default; // because App.jsx is in modules we need to extract the default
...
// this functions are named for the stack traces for easier debugging
...
fastify.get("/react-flight", function reactFlightHandler(request, reply) {
  try {
    ...
    // whenever chunks are available for the user send them right away
    pipe(reply.raw);
  } catch (err) {
